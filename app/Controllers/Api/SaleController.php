...
class SaleController extends BaseController
{
    public function store(): void
    {
        ...
        $stmt = $db->prepare("SELECT id, name, price, is_cake, stock_quantity FROM products WHERE id IN ({$placeholders}) AND is_active = 1");
        ...
        foreach ($items as $item) {
            ...
            if (!$isCake) {
                $stockRequests[$pid] = ($stockRequests[$pid] ?? 0) + $qty;
                if ($stockRequests[$pid] > $availableStock) {
                    $this->jsonError($this->stockConflictMessage($product['name'], $availableStock), 409);
                }
            }
        ...
        $changeGiven = $paymentMethod === 'cash'
            ? round(max(0, $cashTendered - $total), 2)
        ...
        try {
            ...
            foreach ($stockRequests as $productId => $requestedQty) {
                ...
                if ($stockStmt->rowCount() !== 1) {
                    $currentStockStmt = $db->prepare("SELECT stock_quantity FROM products WHERE id = ?");
                    $currentStockStmt->execute([$productId]);
                    $currentProduct = $currentStockStmt->fetch();
                    $availableStock = $currentProduct ? (int)$currentProduct['stock_quantity'] : 0;

                    $db->rollBack();
                    $this->jsonError($this->stockConflictMessage($productMap[$productId]['name'], $availableStock), 409);
                }
            }
        ...
        $dirtyTables = ['transactions', 'products'];
        if ($hasCakeOrders) {
            $dirtyTables[] = 'cake_orders';
        }
        SyncState::markDirty($db, $dirtyTables);
    ...
    private function stockConflictMessage(string $productName, int $availableQty): string
    {
        if ($availableQty <= 0) {
            return $productName . ' is out of stock.';
        }

        return 'Only ' . $availableQty . ' left for ' . $productName . '.';
    }
